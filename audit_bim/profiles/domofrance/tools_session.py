"""Outils MCP du profil Domofrance — désignation de la maquette cible.

Un seul outil ici. Les cinq outils de session — cible analysée, accès, identité,
instantané, téléchargement IFC — viennent du socle partagé, que les trois
profils déclarent.

``set_active_target`` reste propre au profil parce que son équivalent I3F porte
une phase BIM et un système de classification qui appartiennent au référentiel
d'I3F. Le geste métier est le même que chez le profil frère — désigner une cible
et configurer l'authentification, sans prouver l'accès — mais le fichier
appartient à ce profil, et aucun import ne les relie.
"""
...
logger = logging.getLogger("audit_bim.profiles.domofrance.tools_session")
