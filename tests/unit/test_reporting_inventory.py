...
def test_the_document_figures_match_the_measurement(report):
    ...
    assert sum(m["lines"] for m in report["modules"]) == 8308
    assert counts["façade"] == 0
    assert counts["orchestration_i3f"] == 12 and lines["orchestration_i3f"] == 6036
    assert counts["lié_livrable_i3f"] == 9 and lines["lié_livrable_i3f"] == 2265
    ...
    for claim in (
        ...
        "| Lié au livrable I3F par ses appelants | 9 | 2 265 |",
    ):
